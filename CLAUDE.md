...
- Tailwind only generates classes it can see in source, so a class name can never be interpolated from a variable. Map through a lookup of literal classes instead — see `Text`'s `lineClamp`. The reverse also bites: it scans *comments*, so naming a utility in prose mints it. `__tests__` is excluded from the scan for that reason.
- Never set a text colour through opacity (`text-primary/80`). Opacity blends toward the background, so it always reduces contrast — and a hover or disabled state owes the same 4.5:1 as the resting one. The contrast test fails the build on it. Background tints are fine; the text on them is checked separately.
...
- `useControllableState` owns the controlled/uncontrolled pattern; every component uses it and new ones must. The contract it protects: when a consumer supplies `value` and declines the change, the component must not move anyway — `ControlledIgnoresDeclinedChange` on Tabs and Slider guards that.
