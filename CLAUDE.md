...
- Moving focus into an overlay opened by a *click* must be deferred a frame (`requestAnimationFrame`). The browser settles focus onto `<body>` while handling that click, after React's effects have run, so an immediate `.focus()` is silently undone — the symptom is arrow keys and Escape doing nothing, because the panel handling them was never focused. See `ContextMenu` and `Menubar`.
